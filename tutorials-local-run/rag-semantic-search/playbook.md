...
# ------------------------------------------------------------
#   [1] score=0.570  Task decomposition can be done (1) by LLM with simple prompting like...
#   [2] score=0.524  Component One: Planning# A complicated task usually involves many steps...
#   [3] score=0.442  Finite context length: The restricted context capacity limits...
#   [4] score=0.420  Illustration of how HuggingGPT works...
#
# Query: What are the main types of memory in LLM agents?
# ------------------------------------------------------------
#   [1] score=0.636  Memory  Short-term memory: I would consider all the in-context learning...
#   [2] score=0.615  Memory stream: is a long-term memory module (external database)...
#   [3] score=0.590  Comparison of AD, ED, source policy and RL^2 on environments...
#   [4] score=0.572  LLM Powered Autonomous Agents  Date: June 23, 2023...
#
# Query: How does the ReAct framework work?
# ------------------------------------------------------------
#   [1] score=0.396  Case Studies# Scientific Discovery Agent# ChemCrow (Bran et al. 2023)...
#   [2] score=0.378  Self-Reflection# Self-reflection is a vital aspect that allows...
#   [3] score=0.346  Examples of reasoning trajectories for knowledge-intensive tasks...
#   [4] score=0.338  },  #  … same conversation as earlier...
```

> The `score` is cosine similarity — higher means more relevant. Notice the score ranges: memory (0.636) > task decomposition (0.570) > ReAct (0.396). ReAct scores low because it isn't a dominant topic in this document — the chunks returned aren't actually about ReAct. This is what a poor retrieval looks like before it reaches the LLM. In rag-2step, those low-scoring chunks would produce a weak or hallucinated answer.
